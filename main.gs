    ...
    if(!sheet) {
      ...
      if(!sheet) {
        ...
        if(sheet.getLastRow() == 0) {
          ...
          sheet.getRange("A1:G"+(properties.length)).setValues(properties);
...
loadTimesheets = function (exports) {
      ...
      if(!data.signIn || data.signIn === '-') {
        ...
        if(!!this.time) {
          ...
          if( this.message.indexOf('cancel') != -1){
            ...
            this.responder.template("出勤取り消し", username, this.datetimeStr);
          }
        }
      }
    }
        ...
        if(!!this.time) {
          ...
          this.responder.template("退勤更新", username, this.datetimeStr);
           ...
           if( this.message.indexOf('cancel') != -1){
            ...
            this.responder.template("退勤取り消し", username, this.datetimeStr);
           }
        }
      }
    }
  ...
  Timesheets.prototype.actionStartLunch = function(username, message) {
    ...
    if(this.datetime) {
      ...
      if(!data.lunchStart || data.lunchStart === '-') {
        ...
        this.responder.template("ランチ開始", username, this.datetimeStr);
      }
      else {
        // equire time when update
        if(!!this.time) {
          this.storage.set(username, this.datetime, {lunchStart: this.datetime});
          this.responder.template("ランチ開始更新", username, this.datetimeStr);
        }else{
          if( this.message.indexOf('cancel') != -1){
            this.storage.set(username, this.datetime, {lunchStart: '-'});
            this.responder.template("ランチ開始取り消し", username, this.datetimeStr);
          }
        }
      }
    }
  ...
  Timesheets.prototype.actionFinishLunch = function(username, message) {
    if(this.datetime) {
      ...
      if(!data.lunchFinish || data.lunchFinish === '-') {
        ...
        this.responder.template("ランチ終了", username, this.datetimeStr);
      }
      else{
        // Require time when update
        if(!!this.time) {
          this.storage.set(username, this.datetime, {lunchFinish: this.datetime});
          this.responder.template("ランチ終了更新", username, this.datetimeStr);
        }else{
           if( this.message.indexOf('cancel') != -1){
            this.storage.set(username, this.datetime, {lunchFinish: '-'});
            this.responder.template("ランチ終了取り消し", username, this.datetimeStr);
           }
        }
      }
    }
  ...
  Timesheets.prototype.actionCancelLeave = function(username, message) {
    if(this.date) {
      ...
      if(!data.signOut || data.signOut === '-') {
        this.storage.set(username, dateObj, {signIn: null, signOut: null, note: null, lunchStart: null, lunchFinish: null});
        this.responder.template("有給休暇取消", username, DateUtils.format("Y/m/d", dateObj));
      }
    }
  };
  ...
  Timesheets.prototype.actionRemote = function(username, message) {
    if(this.date) {
      ...
      if(!data.signOut || data.signOut === '-') {
        ...
        this.responder.template("リモート", username, DateUtils.format("Y/m/d", dateObj));
      }
    }
  };
  ...
  Timesheets.prototype.actionCancelRemote = function(username, message) {
    if(this.date) {
      ...
      if(!data.signOut || data.signOut === '-') {
        ...
        this.responder.template("リモート取消", username, DateUtils.format("Y/m/d", dateObj));
      }
    }
  };
